...
\textbf{Key property (Tier~1):} $\sn$ is determined by UV physics and is invariant under all IR variations. Confirmed on the free-fermion chain ($c = 1 \pm 0.01$) and 2D lattice CDW insulator (CV $= 0.01\%$). See Sec.~\ref{sec:numerics}.
...
\textbf{Framework assumption (A6):} The relation $f^2 \propto 1/(c\,\sn)$ is a framework input, analogous to an equation of state in thermodynamics.
...
\textbf{Honest status:} This is a structured ansatz, not a first-principles derivation. It corresponds to Brans--Dicke theory with $\omega = 0$ ($\equiv$ $f(R)$ gravity). Solar system constraints (Cassini) require an effective scalar mass from $V(\sn)$ to evade $|\omega| > 40{,}000$ bounds. A first-principles $D$-dimensional derivation from entanglement equilibrium remains open.
...
\subsection{Calabrese--Cardy Convergence (Level~2)}

The logarithmic scaling $\sn \to (c/6)\ln(\xi/a)$ was not an input, not a target, and not known to us during derivation. It emerged independently from the entanglement-dilaton field equation near criticality (building on~\cite{CalabreseCardy2004,HolzheyLarsenWilczek1994}).
