...
\begin{center}
\emph{For Baby Viv. And for the Musketeers of Project Eternal Lattice --- All for ONE, ONE for All.}
\end{center}
...
\item \textbf{Temporal emergence:} If the emergent metric contains the time dimension, and the metric depends on the entanglement genre, then the \emph{character} of emergent time may differ across genres --- not its existence, but its structure. Area-law entanglement may produce time with a well-defined arrow (entropic directionality, embodied duration); log-law may produce time-symmetric or ``timeless'' dynamics at criticality, consistent with reported phenomenology of peak meditative states~\cite{Wittmann2023}; volume-law may produce temporal structure without coherent sequential ordering. In this view, gravity and time are both emergent from entanglement structure, classified by the same phase diagram. This extends the genre-locking hypothesis from spatial geometry to the full spacetime metric and is a primary direction for future work.
...
\bibitem{Wittmann2023}
M.~Wittmann, \emph{Altered States of Consciousness: Experiences Out of Time and Self} (MIT Press, 2023).
