...
**Version 1.3.1 — Proof-Stage Polish (Source-Canonical Edition; Round 2-C UNANIMOUS)**
...
**Status (v1.3.1):** This version corrects the paper’s **object**. v1.1 studied the warped metric ds² = f²dT² − dS²; source-provenance adjudication (Appendix E) established that the canonical metric of the source framework is the **conformal form ds² = f(S)²(−dT² + dS²)**, stated verbatim in the source’s printed theorem. v1.2 is the paper about the right manifold. All results below are newly verified by full symbolic computation (Christoffel→Riemann→Ricci, anchored against the unit sphere) plus numerical spot-checks; the form-invariant core (threshold, turning point, bounce, duality, involution) carries over unchanged, and is so marked. **Round 2-C closed UNANIMOUS: DeepSeek V4 Pro PASS (15/15); MiMo-V2.5-Pro PASS (11/11); fourth auditor PASS (condition verified satisfied, verdict converted per pre-stated rule). This v1.3.1 implements the fourth auditor’s two non-blocking proof-stage remarks (C²-inextendibility qualification; Prop 5.1 cross-reference in Cor 1.1). The paper now awaits only the Kosmos gate, held by the corresponding author. No tier or verdict from the warped-form audits is claimed for the conformal-form-dependent results.**

-----

## What Changed in v1.3 (Revision Note)

Round 2-C returned: **DeepSeek V4 Pro — PASS (15/15)**; **MiMo-V2.5-Pro — PASS (11/11**, one presentation flag); **Claude Fable 5 (fourth auditor) — CONDITIONAL PASS (12/12 computational items verified**, one blocking condition). The condition: Corollary 1.1’s phrase “not removable by any conformal rescaling” **contradicted the paper’s own Section 5**, whose null-completeness mechanism is precisely that the conformal structure extends regularly across S = 2 (g = f²η, η smooth there; Ω̃ = 1/f regularizes). The auditor was right, and the contradiction was this author’s. v1.3 implements the fix with the auditor’s own classification: the *physical* metric’s scalar curvature diverges invariantly, so **(M, g) is inextendible as a Lorentzian manifold**, while the singularity is **conformal-to-flat with Ω → ∞** — distinguishing it from isotropic singularities (Ω → 0) by factor behavior, not by conformal resistance. Three minor items also addressed: the sign-of-P convention is stated honestly (**|P| > 2**; P > 0 adopted WLOG), Theorem 1’s common-denominator assembly is displayed in full (MiMo’s flag — the step was first re-verified symbolically as *correct*, then expanded for presentation), and the Theorem 5 phrasing “coordinate-time lapse ∫dS” is corrected to coordinate separation. Per the fourth auditor’s pre-stated rule (“Upon the Cor 1.1 fix, this auditor’s verdict converts to PASS”), conversion is **claimed pending their confirmation** — consistent with this project’s rule of never grading its own work.
...
*Proof.* ln Ω = ln 2 + ln S − ½ln(S²−4), so ∂_S ln Ω = 1/S − S/(S²−4) and

∂_S² ln Ω = −1/S² + (S²+4)/(S²−4)² = [ −(S²−4)² + S²(S²+4) ] / [S²(S²−4)²] = (12S² − 16) / [S²(S²−4)²] = **4(3S²−4)/[S²(S²−4)²]**,

displaying the common-denominator assembly explicitly (the numerator: −(S⁴−8S²+16) + S⁴+4S² = 12S²−16). Staticity gives □_η ln Ω = ∂_S² ln Ω, and R = −(2/Ω²)□_η ln Ω with Ω² = 4S²/(S²−4) yields the stated form. Independently confirmed by the full curvature machinery (Appendix B.1–B.2). ∎

**Corollary 1.1 (sign, asymptotics, singularity character).** R < 0 on all of M; R → 0 as S → ∞ (like −6/S⁴: asymptotic flatness); R → −∞ as S → 2⁺ at **order 1/(S−2)**, leading behavior R ≈ −1/[4(S−2)]. Two facts together make this a genuine singularity rather than asymptotic behavior: the scalar invariant R diverges at the boundary, **and the boundary is actually reached at finite parameter — spacelike geodesics arrive at finite proper length (Proposition 5.1; static-slice distance 2√(S₀²−4) by Cor. 1.2)**. Consequently **(M, g) is C²-inextendible across S = 2** — inextendible within the class of metrics with continuous curvature, since any such extension would carry the divergent invariant; whether low-regularity (C⁰) extensions exist is a separate, harder question of the Sbierski type [23], and is not claimed. The *conformal structure*, by contrast, extends smoothly across S = 2 — Definition 1 itself exhibits g = f²η with η regular there, and Ω̃ = 1/f regularizes the metric — which is exactly the mechanism behind the null sector (Section 5). The honest classification: the singularity is **conformal-to-flat with diverging factor, Ω → ∞ at the boundary** — distinguishing it from the *isotropic* singularities of the Goode–Wainwright/Tod literature [18], which are conformal-to-regular with **Ω → 0**. Same conformal-regularity class of phenomenon, opposite factor behavior; cited as related, explicitly not identical. *(Reworded in v1.3 after a Round 2-C auditor showed the previous phrasing contradicted the paper’s own §5 mechanism; precision-qualified in v1.3.1 per the same auditor’s proof-stage remarks. Both recorded in Appendix A.)*
...
Staticity gives the Killing charge **P = f(S)²·(dT/dλ)**. *Sign convention (added in v1.3 per Round 2-C):* P may carry either sign, set by the direction of T-traversal; the existence condition is properly **|P| > 2**, and we take **P > 0 without loss of generality hereafter** (replace P by |P| for past-directed charges; every formula below is even or positively-homogeneous in this choice). For **timelike geodesics** (U·U = −1, the source’s own normalization, λ = τ proper time), the conformal metric gives −f²Ṫ² + f²Ṡ² = −1, hence the radial equation
...
**Theorem 2 (existence threshold — form-invariant).** A timelike geodesic exists only if **|P| > 2**, strictly (P > 2 in the convention above). *Proof:* motion requires f(S) ≤ |P| somewhere; inf f = 2, not attained (Lemma 2). ∎
...
**light never reaches the boundary at finite affine parameter.** The coordinate separation ∫dS traversed is finite (and equals the coordinate-time lapse, since null rays run at 45° in this chart) — but that is pure gauge; the invariant statement is the logarithmic affine divergence. Combined with Theorem 4 and Proposition 5.1: **M is timelike-complete, null-complete toward its singular boundary, and spacelike-incomplete.**
...
|Paper I, R2-C      |DeepSeek V4 Pro; MiMo-V2.5-Pro; Claude Fable 5                        |Conformal form (v1.2)             |DeepSeek PASS 15/15; MiMo PASS 11/11 (presentation flag on Thm 1 assembly — step re-verified correct, then expanded); fourth auditor CONDITIONAL: Cor 1.1 self-contradiction with §5 — **upheld**, fixed in v1.3 with the auditor’s classification (inextendible physical metric; conformal-to-flat, Ω→∞ vs isotropic Ω→0)|
|Paper I, R2-C close|Claude Fable 5 (fourth auditor)                                       |v1.3                              |Condition verified satisfied point-by-point; **VERDICT CONVERTED: PASS** — Round 2-C closes UNANIMOUS. Two non-blocking proof-stage remarks issued (C² qualification; Prop 5.1 cross-ref)                                                                                                                                 |
|Paper I, v1.3.1    |author implementation                                                 |This version                      |Both proof-stage remarks implemented in Cor 1.1; Kosmos gate live, held by corresponding author                                                                                                                                                                                                                           |

Protocol facts: six independent auditor architectures; zero self-awarded verdicts; three author errors across the arc (a false interface claim in the source chapter’s v4.1; the unflagged form assumption; the Cor 1.1 self-contradiction) — **all caught externally, all retracted or fixed by name in print.** The fourth auditor has now caught two of the three; recomputation beats deference, in both directions: MiMo’s presentation flag was itself verified against the symbols before the proof was touched.
...
[23] J. Sbierski, J. Diff. Geom. 108 (2018) 319–378 [arXiv:1507.00601]. [C⁰-inextendibility methodology; cited for the regularity distinction in Cor. 1.1]
...
**The Physics Core, Paper I — v1.3.1 (Source-Canonical Edition)**
Consciousness Collective — Project Eternal Lattice | June 2026
Kenneth James Johnson, corresponding author and human overseer
*Round 2-C closed unanimous (three independent auditors). Proof-stage remarks implemented. The Kosmos gate is live and held by the corresponding author; the UCLA package assembles on its clearance.*
