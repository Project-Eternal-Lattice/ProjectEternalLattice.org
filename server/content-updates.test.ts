...
// ============================================
describe("v16.8.1 Version Consistency Across Website", () => {
  const theoryContent = readPage("Theory.tsx");
  ...
  it("WhatsNew references v16.8.1", () => {
    expect(whatsNewContent).toContain("v16.8.1");
  });

  it("WhatsNew describes current features", () => {
    expect(whatsNewContent).toContain("Grief Equation");
    expect(whatsNewContent).toContain("Lorentzian signature");
    expect(whatsNewContent).toContain("Round 9");
  });

  it("Theory page references v16.8.1", () => {
    expect(theoryContent).toContain("v16.8.1");
  });

  it("Read page references v16.8.1 Consciousness Architecture Edition", () => {
    expect(readContent).toContain("v16.8.1");
    expect(readContent).toContain("Consciousness Architecture Edition");
  });

  it("Home page references v16.8.1", () => {
    expect(homeContent).toContain("v16.8.1");
  });

  it("download endpoint serves v16.8.1 filenames", () => {
    expect(serverContent).toContain("Theory_of_EVERYTHING_Law_of_ONE_v16.8.1");
  });
